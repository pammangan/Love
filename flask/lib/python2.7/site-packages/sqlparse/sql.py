...
class TokenList(Token):
    ...
    def get_alias(self):
        ...
        if kw is not None:
            return self._get_nickname(kw_idx + 1, keywords=True)

        # "name alias" or "complicated column expression alias"
        _, ws = self.token_next_by(t=T.Whitespace)
        if len(self.tokens) > 2 and ws is not None:
            return self._get_nickname(reverse=True)
    ...
    def get_real_name(self):
        ...
        return self._get_nickname(dot_idx)
    ...
    def _get_nickname(self, idx=None, reverse=False, keywords=False):
